...
| Database | PostgreSQL | System of record for all WEMA-owned entities |
| API style | REST | Versioned, resource-oriented |
| Background processing | Node worker processes (queue-backed) | Handles `UPDATE_WONDER`, `SEND_WHATSAPP_ALERT`, etc. |
| Wonder integration | Dedicated adapter module | Isolates all Wonder-specific request/response mapping |
| Logging | Pino | Structured JSON logs, sensitive-field redaction |
| Auth | JWT | Internal portals (psychologist, admin) only — patient tablet uses device-level trust, not per-user login |
| Authorization | Role-based access control (RBAC) | Roles: `psychologist`, `wonder_service` |
